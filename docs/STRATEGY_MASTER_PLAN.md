...
### 📅 Phase 3: 통합 인텔리전스 & 워크플로우 자동화 (완료)
*   **성과 요약:**
    - 뉴스/공시 요약을 벡터 스토어와 밸류체인 그래프로 연결해 Text Intelligence 체계 구축.
    - 툴 호출 시 채팅 스트림 내 위젯으로 노출되는 Generative UI 패턴 정착(News/Peer/Value Chain).
    - Proactive Signals, Paywall/Teaser, LightMem 연동까지 완료해 “궁금하면 불러 쓰는 워크플로우” 실현.
*   **다음 적용 범위:** 밸류체인 관계 DB 영속화·Ticker 보정·Skeleton UX 등 유지보수성 확보(적용 완료) → Phase 4 전환 준비 완료.

### 📅 Phase 4: Synthesis & Action (준비 중)
*   **목표:** 파편화된 인사이트(뉴스·차트·밸류체인·공시)를 자동으로 종합해 “보고서 초안”을 명령 한 번에 생성.
*   **핵심 미션:** TOOL_REPORT(AI 투자 메모) 오케스트레이터 구축 → 데이터 수집(Agent) → Structured Markdown → Export.

#### Phase 4 우선 과제
| 순서 | 작업명 | 목표 결과물 | 비즈니스 임팩트 |
| :--- | :--- | :--- | :--- |
| 1 | Report Orchestrator (백엔드) | 뉴스를 비롯한 툴 결과를 병렬 수집 후 Markdown 메모 생성 | 애널리스트 초안 작성 시간 90% 절감 |
| 2 | Tool Streaming/Progress UX | “뉴스 수집 → 경쟁사 비교 → 보고서 작성” 단계 표시 및 타닥 스트림 | 10~20초 작업도 “일하고 있다”는 체감 제공 |
| 3 | ReportPanel & Export | `react-markdown` 렌더 + PDF 다운로드 + 컴플라이언스 문구 | 공유/상신 최적화 → Enterprise 락인 강화 |

> **Phase 4 메모:** Semantic Router·tool_registry 구조 덕분에 TOOL_REPORT 추가 시 최소 수정으로 확장 가능. 투자 메모 생성과 동시에 LightMem/툴 컨텍스트에 요약을 주입해 다음 대화에 활용한다.
