...
migrate:
	...
	docker-compose run --rm api python scripts/migrate_schema.py
...
seed:
	...
	docker-compose run --rm api python scripts/seed_data.py --days-back 1
	docker-compose run --rm api python scripts/seed_news.py
