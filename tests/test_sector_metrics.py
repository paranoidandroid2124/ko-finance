...
def test_sector_metrics_pipeline_generates_daily_and_window_rows(session: Session):
    sectors = ensure_sector_catalog(session)
    semiconductor = sectors['semiconductor']

    as_of_day = date(2025, 1, 15)
    _add_daily_history(session, semiconductor, days=14, end_day=as_of_day)
    ...
    daily = compute_sector_daily_metrics(session, as_of_day, as_of_day)
    session.commit()
    ...
    records = compute_sector_window_metrics(session, as_of_day, window_days=(7, 30))
    session.commit()
