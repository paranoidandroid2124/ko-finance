...
def test_render_watchlist_slack_contains_top_tickers() -> None:
    sample_payload = {
        "windowMinutes": 60,
        "summary": {
            "totalDeliveries": 3,
        ...
        "items": [
            {
                ...
                "channel": "slack",
                "headline": "0001 headline",
                "summary": "0001 headline",
                "message": "alert",
                "ruleTags": [],
                "ruleTickers": [],
                "eventType": "news",
    ...
    text = watchlist_service.render_watchlist_slack(sample_payload)
    assert "0001" in text
    assert "Watchlist Radar" in text


def test_render_watchlist_email_uses_template() -> None:
    # Point template directory to the real email templates.
    sample_payload = {
    ...
    html = watchlist_service.render_watchlist_email(sample_payload)
    assert "Watchlist Radar" in html
    assert "0001 headline" in html


def test_dispatch_watchlist_digest(monkeypatch: pytest.MonkeyPatch) -> None:
    sample_payload = {
        "generatedAt": "2025-01-01T12:00:00+00:00",
        "windowMinutes": 60,
    ...
    def fake_dispatch(channel: str, message: str, target=None, *, targets=None, metadata=None, template=None):
        delivered = len(targets or [])
        return NotificationResult(status="delivered", delivered=delivered, failed=0, metadata=metadata)
    ...
    result = watchlist_service.dispatch_watchlist_digest(
        ...
        window_minutes=60,
        limit=10,
        slack_targets=["https://hooks.slack.com/services/demo"],
        email_targets=["ops@example.com"],
    )

    assert result["results"]
    assert any(entry["channel"] == "email" for entry in result["results"])
    assert any(entry["channel"] == "slack" for entry in result["results"])
