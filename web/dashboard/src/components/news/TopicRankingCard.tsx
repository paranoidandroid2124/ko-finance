...
export function TopicRankingCard({ topics }: { topics: Topic[] }) {
  return (
    <div className="rounded-xl border border-border-light bg-background-cardLight p-4 shadow-card transition-colors dark:border-border-dark dark:bg-background-cardDark">
      ...
      <p className="text-xs text-text-secondaryLight dark:text-text-secondaryDark">최근 15분 기준 상위 키워드</p>
      <ul className="mt-3 space-y-2">
        {topics.map((topic, index) => (
          <li key={topic.name} className="flex items-center justify-between rounded-lg border border-border-light/70 px-3 py-2 text-sm dark:border-border-dark/70">
            <div className="flex items-center gap-3">
              <span className="text-xs font-semibold text-text-secondaryLight dark:text-text-secondaryDark">{index + 1}</span>
              <span className="font-medium">{topic.name}</span>
            </div>
            <span className={`text-xs font-semibold ${sentimentColor[topic.sentiment]}`}>{topic.change}</span>
          </li>
        ))}
      </ul>
    </div>
